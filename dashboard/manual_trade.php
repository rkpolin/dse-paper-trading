...
$selectedBuyPrice = isset($stockPriceMap[$selectedBuySymbol]['close_price']) ? $stockPriceMap[$selectedBuySymbol]['close_price'] : null;
$selectedBuyDateText = (string)($stockPriceMap[$selectedBuySymbol]['latest_trade_date'] ?? 'N/A');
$prefillBuyPrice = is_numeric($_GET['target_price'] ?? null)
    ? (float)$_GET['target_price']
    : ($selectedBuyPrice ?? 0.0);
$prefillBuyQuantity = is_numeric($_GET['quantity'] ?? null) && (int)$_GET['quantity'] > 0
    ? (int)$_GET['quantity']
    : 1;
$prefillBuyReason = trim((string)($_GET['reason'] ?? 'MANUAL_BUY'));
if ($prefillBuyReason === '') {
    $prefillBuyReason = 'MANUAL_BUY';
}
$prefillBuyDate = trim((string)($_GET['requested_date'] ?? $defaultBuyDate));
if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $prefillBuyDate)) {
    $prefillBuyDate = $defaultBuyDate;
}
    ...
    <div class="panel" id="buy-form">
        ...
        <form method="post">
                ...
                <select id="buy-symbol" name="symbol" required>
                    ...
                    <?php foreach ($stockRows as $row): ?>
                        <option
                            ...
                            <?= (string)$row['symbol'] === $selectedBuySymbol ? 'selected' : '' ?>
                        >
            ...
            <div class="field">
                ...
                <div id="buy-current-date" class="readonly-box"><?= h($selectedBuyDateText) ?></div>
            </div>
            <div class="field">
                <label for="buy-date">Order Start Date</label>
                <input id="buy-date" name="requested_date" type="date" value="<?= h($prefillBuyDate) ?>" required>
            </div>
            <div class="field">
                <label for="buy-price">My Buy Price</label>
                <input id="buy-price" name="target_price" type="number" min="0.01" step="0.01" value="<?= $prefillBuyPrice > 0 ? h(number_format($prefillBuyPrice, 2, '.', '')) : '' ?>" required>
            </div>
            <div class="field">
                <label for="buy-quantity">Quantity</label>
                <input id="buy-quantity" name="quantity" type="number" min="1" step="1" value="<?= h((string)$prefillBuyQuantity) ?>" required>
            </div>
            <div class="field">
                <label for="buy-reason">Reason</label>
                <input id="buy-reason" name="reason" type="text" maxlength="50" value="<?= h($prefillBuyReason) ?>">
            </div>
