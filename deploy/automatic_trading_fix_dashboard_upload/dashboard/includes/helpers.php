    ...
    $row = fetch_one(
        ...
        "SELECT run_id
         ...
         WHERE source = 'github_actions_python_engine'
           AND status = 'SUCCESS'
           AND latest_data_date IS NOT NULL
           AND EXISTS (
               SELECT 1
               FROM portfolio_snapshots
               WHERE portfolio_snapshots.run_id = system_runs.run_id
           )
         ORDER BY latest_data_date DESC, created_at DESC
         LIMIT 1"
    );
        ...
        "SELECT run_id
         ...
         WHERE source = 'github_actions_intraday_engine'
           ...
           AND EXISTS (
               ...
               WHERE intraday_snapshots.run_id = system_runs.run_id
           )
