        ...
        <?php foreach ($signals as $row): ?>
            ...
            <tr>
                ...
                <td class="detail-cell wide-cell">
                    ...
                    <div><strong>বাংলা:</strong> <?= h($explanationBn['why']) ?></div>
                    <div><strong>RSI:</strong> <?= h($explanation['rsi']) ?></div>
                    <div><strong>RSI বাংলা:</strong> <?= h($explanationBn['rsi']) ?></div>
                    <div><strong>SMA20/SMA50:</strong> <?= h($explanation['trend']) ?></div>
                    <div><strong>Trend বাংলা:</strong> <?= h($explanationBn['trend']) ?></div>
                    <div><strong>Volume:</strong> <?= h($explanation['volume']) ?></div>
                    <div><strong>Volume বাংলা:</strong> <?= h($explanationBn['volume']) ?></div>
                    <div><strong>Momentum:</strong> <?= h($explanation['momentum']) ?></div>
                    <div><strong>Momentum বাংলা:</strong> <?= h($explanationBn['momentum']) ?></div>
                    <div><strong>Pump Risk বাংলা:</strong> <?= h($explanationBn['pump_risk']) ?></div>
                    <div><strong>Entry Zone বাংলা:</strong> <?= h($explanationBn['entry']) ?></div>
