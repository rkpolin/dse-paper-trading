...
def test_load_prices_skips_latest_page_when_configured(monkeypatch, tmp_path: Path) -> None:
    csv_path = tmp_path / "demo.csv"
    csv_path.write_text(
        "\n".join(
            [
    ...
    config = SimpleNamespace(
        ...
        dse_archive_lookback_days=120,
        dse_symbols=(),
        dse_skip_latest_page=True,
        merge_dse_with_csv=True,
    )

    def fail_if_called(*args, **kwargs):
        raise AssertionError("latest page should be skipped")

    monkeypatch.setattr(run_engine, "fetch_latest_dse_prices", fail_if_called)
    monkeypatch.setattr(run_engine, "fetch_dse_archive_prices", lambda *args, **kwargs: archive_df)
