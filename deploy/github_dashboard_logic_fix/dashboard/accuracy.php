...
$accuracyTotals = $latestRunId ? (fetch_one(
    $pdo,
    "SELECT
        SUM(status = 'CORRECT') AS correct_count,
        SUM(status = 'WRONG') AS wrong_count
...
$completedCount = (int)($accuracyTotals['correct_count'] ?? 0) + (int)($accuracyTotals['wrong_count'] ?? 0);
$completedAccuracy = $completedCount > 0 ? (int)($accuracyTotals['correct_count'] ?? 0) / $completedCount : null;

$pageTitle = 'Accuracy';
require __DIR__ . '/includes/header.php';
?>
<section class="panel">
    <strong>Completed accuracy:</strong> <?= pct_or_na($completedAccuracy, 'Not enough data yet') ?>
    <div class="muted small-text">Only CORRECT and WRONG evaluations are counted. BUY signal is evaluated after 5 trading days.</div>
</section>
