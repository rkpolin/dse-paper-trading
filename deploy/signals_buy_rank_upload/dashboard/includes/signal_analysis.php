...
function calculate_buy_rank_display(
    array $signal,
    array $entryZone,
    array $pumpRisk,
    float $affordabilityScore = 50.0
): array {
    $signalType = (string)($signal['signal_type'] ?? '');
    if ($signalType !== 'BUY' || !($entryZone['available'] ?? false)) {
        return [
            'eligible' => false,
            'eligible_for_focus' => false,
            'score' => 0.0,
            'label' => 'N/A',
            'reason' => 'Only BUY signals with a valid entry zone are ranked.',
        ];
    }

    $current = signal_number($signal, 'close_price');
    $target2 = (float)($entryZone['target2'] ?? 0.0);
    $stopLoss = (float)($entryZone['stop_loss'] ?? 0.0);
    $confidencePct = clamp_number(signal_number($signal, 'confidence') * 100.0, 0.0, 100.0);
    $pumpSafetyPct = clamp_number(100.0 - (float)($pumpRisk['score'] ?? 0.0), 0.0, 100.0);
    $affordabilityPct = clamp_number($affordabilityScore, 0.0, 100.0);

    $action = (string)($entryZone['action'] ?? 'N/A');
    $actionPoints = match ($action) {
        'BUY NOW' => 100.0,
        'WATCH / SMALL ENTRY' => 65.0,
        'WAIT' => 30.0,
        'AVOID / WAIT FOR PULLBACK' => 5.0,
        'AVOID' => 0.0,
        default => 10.0,
    };
    ...
    $riskPct = 0.0;
    if ($current > 0.0) {
        $upsidePct = max(0.0, ($target2 - $current) / $current);
        $riskPct = max(0.0, ($current - $stopLoss) / $current);
    }
    $rewardRiskRatio = $riskPct > 0 ? $upsidePct / $riskPct : 0.0;
    $rewardRiskPoints = clamp_number(($rewardRiskRatio / 2.5) * 100.0, 0.0, 100.0);

    $score = (
        ($confidencePct * 0.35) +
        ($actionPoints * 0.25) +
        ($pumpSafetyPct * 0.20) +
        ($rewardRiskPoints * 0.10) +
        ($affordabilityPct * 0.10)
    );
    $score = round(clamp_number($score, 0.0, 100.0), 2);
    ...
    $reasonParts = [
        'Signal Score ' . number_format($confidencePct, 0) . '/100',
        'Entry Action ' . $action,
        'Pump Risk ' . (string)($pumpRisk['level'] ?? 'LOW'),
    ];
    if ($current > 0.0 && $target2 > 0.0) {
        $reasonParts[] = 'Target 2 upside ' . number_format($upsidePct * 100.0, 1) . '%';
    }
    $reasonParts[] = 'Affordability ' . number_format($affordabilityPct, 0) . '/100';

    $eligibleForFocus = in_array($action, ['BUY NOW', 'WATCH / SMALL ENTRY'], true)
        && !in_array((string)($pumpRisk['level'] ?? ''), ['HIGH', 'EXTREME'], true);

    return [
        'eligible' => true,
        'eligible_for_focus' => $eligibleForFocus,
        'score' => $score,
        'label' => $label,
